jQuery(function($) {
    ...
    //added 08.02.2021 update bug
    if(url.indexOf("/form/dariah-eldah-consent-form-wizard") >= 0){
        var divId = "edit-actions";
        var inputId = "edit-actions-submit";
        var inputValue = "Next page";
       
        if($('#'+divId).hasClass('js-webform-states-hidden')) {
            $('#'+divId).removeClass('js-webform-states-hidden');
            $('#'+divId).addClass('js-webform-states');
             $('#'+divId+' input#'+inputId).val(inputValue);
        }
    }
    ...
    $(document).ready(function() {
         ...
         */
        $('.language-switcher-language-session-dariah').on('click', function(event) {
            let lng = $(this).data('lang');
            console.log(lng);
            $.ajax({
                url: '/dariah_change_lng/'+lng,
                type: "POST",
